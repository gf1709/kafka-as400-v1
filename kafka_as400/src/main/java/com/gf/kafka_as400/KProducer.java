...
public class KProducer {
	...
	public static void produce() {
		...
		props.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, "fcvlkas1.fc.crtnet:9092");
			...
			producer.send(new ProducerRecord<>(TOPIC, key, val),
					(event, ex) -> {
						if (ex != null)
							...
							log.info(String.format("Produced event to topic %s: key = %-10s value = %s%n",TOPIC, key, val));
					});
